...
#from onmt.encoders.transformerh import TransformerEncoder_h
...
def build_encoder(opt, embeddings):
    ...
    elif opt.encoder_type == "transformerh":
        return TransformerEncoder_h(opt.enc_layers, opt.enc_rnn_size,
                                  opt.heads, opt.transformer_ff,
                                  opt.dropout, embeddings, opt.c_val)
...
def load_test_model(opt, dummy_opt, model_path=None):
    ...
    for arg in dummy_opt:
        if arg not in model_opt.__dict__.keys():
